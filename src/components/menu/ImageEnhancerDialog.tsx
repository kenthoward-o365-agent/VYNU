...
export default function ImageEnhancerDialog({ open, onOpenChange, venueId, items, scopedItemIds, onComplete }: Props) {
  ...
  // Preflight: provider status, the venue's AI-generation allowance, and how
  // many missing images the shared dish library already covers for free.
  const [preflight, setPreflight] = useState<{
    configured: boolean; capUsed: number; capLimit: number;
    missingImages: number; libraryCoverage: number;
  } | null>(null);
  useEffect(() => {
    if (!open) return;
    let cancelled = false;
    supabase.functions
      .invoke("batch-generate-images", { body: { venueId, probe: true } })
      .then(({ data }) => {
        if (!cancelled && data && typeof data.configured === "boolean") setPreflight(data);
      })
      .catch(() => {});
            ...
            {!genProcessing && genResults.length === 0 && (
              <div className="space-y-4 py-4">
                <div className="text-center space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Dishes already in the shared VYNU library get their image instantly and free;
                    the rest are generated by AI.
                  </p>
                </div>
                {preflight && missingImageItems.length > 0 && (
                  <div className="rounded-lg border border-border p-3 text-xs space-y-1">
                    <p className="text-foreground">
                      <span className="font-medium">{preflight.libraryCoverage}</span> of{" "}
                      <span className="font-medium">{preflight.missingImages}</span> missing images are
                      covered by the shared dish library (free).
                    </p>
                    <p className="text-muted-foreground">
                      AI generations used: {preflight.capUsed} / {preflight.capLimit}
                    </p>
                    {!preflight.configured && (
                      <p className="text-destructive">
                        AI generation is temporarily unavailable — the platform's image provider is
                        being set up. Library images will still be applied.
                      </p>
                    )}
                  </div>
                )}
                {missingImageItems.length > 0 && (
                  <Button
                    ...
                    className="w-full"
                    disabled={preflight ? (!preflight.configured && preflight.libraryCoverage === 0) : false}
                  >
